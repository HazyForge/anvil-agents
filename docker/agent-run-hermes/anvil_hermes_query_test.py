...
class Agent:
    ...
    def run_conversation(self, *args, **kwargs):
        if self.reasoning_callback is not None:
            self.reasoning_callback("DEBUG_REASONING_CALLBACK")
        print("DEBUG_REASONING_DISPLAY")
        os.write(1, b"DEBUG_FD_OUTPUT\n")
...
def main(**kwargs):
    ...
    print("NATIVE_FRAGMENT_WITHOUT_NEWLINE", end="")
...
class HermesQueryTests(unittest.TestCase):
    def envelope(self, output):
        ...
        for line in output.splitlines():
            try:
                ...
                continue
            if isinstance(item, dict) and item.get("type") in ("anvil.hermes.final", "anvil.hermes.error"):
                records.append(item)
        self.assertEqual(len(records), 1, "missing or duplicated standalone envelope")
        return records[0]
    ...
    def run_helper(self, result, exit_code=0, additional=None, native=False, credentials=True):
        with tempfile.TemporaryDirectory() as directory:
            ...
            if credentials:
                self.assertIn("DEBUG_REASONING_CALLBACK", process.stdout)
                self.assertIn("DEBUG_REASONING_DISPLAY", process.stdout)
                self.assertIn("DEBUG_FD_OUTPUT", process.stdout)
            self.assertNotIn("PRIVATE_USER_PROMPT", " ".join(command))
            return process, kwargs

    def test_only_native_final_answer_is_framed(self):
        process, kwargs = self.run_helper({"final_response":"Exact **final**\nUnicode: café", "reasoning_content":"DEBUG_REASONING", "messages":[{"role":"tool","content":"tool value"}]}, additional=["--model","fixture-model","--provider","fixture-provider","--query","ignored"])
        self.assertEqual(process.returncode, 0, process.stderr)
        self.assertEqual(self.envelope(process.stdout), {"type":"anvil.hermes.final", "version":1, "role":"assistant", "text":"Exact **final**\nUnicode: café"})
        self.assertEqual(process.stdout.count('"type": "anvil.hermes.final"'), 1)
    ...
    def test_failures_partial_and_missing_final_are_not_answers(self):
        for result, code in [({"final_response":"failure diagnostic","failed":True},0), ({"final_response":"incomplete","partial":True},0), ({"messages":[{"role":"assistant","content":"intermediate"}]},0), ({"final_response":{"reasoning":"private"}},0), ({"final_response":""},0), ({"final_response":"valid but native exit failed"},7)]:
            with self.subTest(result=result, code=code):
                ...
                self.assertNotIn('"type": "anvil.hermes.final"', process.stdout)
    ...
    def test_provider_setup_failure_has_safe_typed_diagnosis(self):
        ...
        self.assertEqual(self.envelope(process.stdout), {"type":"anvil.hermes.error", "version":1, "code":"provider_configuration_unavailable"})
    ...
    def test_pinned_native_quiet_mode_contract(self):
        for result, success in [({"final_response":"NATIVE_FINAL_ONLY", "reasoning_content":"DEBUG_REASONING"},True), ({"final_response":"not a success", "failed":True},False), ({"final_response":"partial", "partial":True},False)]:
            with self.subTest(result=result):
                process, _ = self.run_helper(result, native=True)
                if success:
                    self.assertEqual(process.returncode, 0, process.stderr)
                    self.assertEqual(self.envelope(process.stdout)["text"], "NATIVE_FINAL_ONLY")
                else:
                    self.assertNotEqual(process.returncode, 0)
                    self.assertNotIn('"type": "anvil.hermes.final"', process.stdout)
