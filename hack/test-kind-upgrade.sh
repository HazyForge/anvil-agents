...
if [[ -n "${baseline_ref}" ]]; then
	...
	for crd in "${root_dir}"/config/crd/bases/*.yaml; do
		case "${crd}" in
			...
			*_agentharnessprofiles.yaml|*_agentskills.yaml|*_agentskillsets.yaml|*_agenttools.yaml|*_agenttoolsets.yaml|*_agentmcpservers.yaml|*_agentmcpsets.yaml|*_adversesignals.yaml|*_agentauthsessions.yaml) continue ;;
		esac
...
[[ "${upgraded_count}" -eq 16 ]] || {
	echo "upgrade rendered ${upgraded_count} agent CRDs, want 16" >&2
	exit 1
}
...
[[ "${retained_count}" -eq 16 ]] || {
	echo "upgrade uninstall retained ${retained_count} agent CRDs, want 16" >&2
	exit 1
}
for resource in \
...
printf 'Seven-to-sixteen CRD Helm upgrade contract passed\n'
