...
Anvil Agents uses namespaced resources for reusable configuration:
...
| `AgentExternalTrigger` | signed inbound GitHub webhook receivers, delivery targets, and the controller-owned Gateway API HTTPRoute in the install namespace (opt-in API gate) | Secret bytes in status; Gateway objects; non-GitHub sources in this slice |
