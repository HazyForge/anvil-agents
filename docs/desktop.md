...
An agent's cluster harness — managers included — can be migrated from Chat
...
binding (`PATCH /api/v1/namespaces/{ns}/agent-run-profiles/{name}/harness`).
It is a lightweight rebind, not a memory migration: skills, tools, and scope
are preserved, the next chat turn resolves the new harness, and the agent
starts with a fresh home on the target harness. Harness-local durable homes
and internal harness memory (Codex home, Grok home, Pi home, …) are not
migrated. GitOps-owned or non-console-managed profiles stay read-only, and the
picker requires `composition.writeEnabled` on the API.
