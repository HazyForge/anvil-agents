...
func main() {
	...
	flag.StringVar(&options.SubstrateEndpoint, "substrate-endpoint", options.SubstrateEndpoint, "atenet-router origin for live actor dispatch (Kind-local for the spike). Empty keeps the SubstrateActor hold.")
	flag.StringVar(&options.SubstrateAtespace, "substrate-atespace", options.SubstrateAtespace, "Substrate atespace for live chat actors (empty means the substrate default).")
	flag.StringVar(&options.SubstrateActorTemplate, "substrate-actor-template", options.SubstrateActorTemplate, "ActorTemplate namespace/name for live actor creates through the control shim.")
	flag.StringVar(&options.SubstrateShimEndpoint, "substrate-shim-endpoint", options.SubstrateShimEndpoint, "cmd/substrate-ate-shim origin backing live create/suspend/pause. Empty keeps those operations failing closed.")
