...
func (app App) runCreate(ctx context.Context, kubeOptions KubeOptions, args []string) error {
	...
	flags.StringVar(&options.purpose, "purpose", options.purpose, "Run purpose: manual, adverseSituation, or scheduledHealthCheck (chained and interactive are not public create).")
...
func (app App) buildRun(options createOptions) (*agentsv1alpha1.AgentRun, error) {
	...
	if reason := purpose.PublicCreateError(); reason != "" {
		return nil, &usageError{message: reason}
	}
