...
type Config struct {
	...
	// Chat gates standing-chat and Anvil council endpoints. Disabled by default.
	Chat ChatConfig `json:"chat"`
}

// ChatConfig controls chat/memory endpoints. The PostgreSQL URI is loaded from
// ANVIL_AGENTS_CHAT_DATABASE_URL, never from this file.
...
func (config Config) Validate() error {
	...
	for i, binding := range config.Authorization.Bindings {
		...
		for _, permission := range binding.Permissions {
			...
			if (permission == PermissionChatRead || permission == PermissionChatWrite) && !config.Chat.Enabled {
