...
func (server *Server) routes() http.Handler {
	...
	server.registerCouncilRoutes(mux)
...
func (server *Server) handleUIConfig(writer http.ResponseWriter, _ *http.Request) {
	...
	writeJSON(writer, http.StatusOK, map[string]any{
		...
		"controls": map[string]any{
			"readEnabled":  server.config.Controls.ReadEnabled,
			"writeEnabled": server.config.Controls.WriteEnabled,
		},
		"runs": map[string]any{
		...
		"chat": map[string]any{
			...
			"councilEnabled": server.config.Chat.Enabled,
			"path":           "/chat",
			"councilPath":    "/council",
		},
	})
...
func (server *Server) handleReady(writer http.ResponseWriter, request *http.Request) {
	...
	if server.config.Chat.Enabled {
		...
		if err := server.chatStore.Ping(request.Context()); err != nil {
			writeAPIError(writer, http.StatusServiceUnavailable, "chat_unavailable", "standing-chat store is unavailable")
			return
		}
	}
