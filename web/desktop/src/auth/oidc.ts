...
const PKCE_KEY = "anvil-agents-desktop.pkce";
const RETURN_KEY = "anvil-agents-desktop.returnTo";
...
async function exchangeToken(config: UIConfig, body: URLSearchParams): Promise<Session> {
  const discovery = await discover(config.oidc.issuer);
