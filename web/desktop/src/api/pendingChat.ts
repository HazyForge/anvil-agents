...
export type PendingChatSend = {content: string; id: string; threadID: string};
...
export function rememberPendingSend(namespace: string, threadID: string, content: string): PendingChatSend {
  const old = readPendingSend(namespace, threadID);
  if (old?.content === content && old.threadID === threadID) return old;
  const next = {content, threadID, id: crypto.randomUUID()};
