/**
 * Markdown rendering smoke tests for Anvil Agents Desktop chat transcripts.
 *
 * Archived / historical agent text is rendered as sanitized GFM Markdown via
 * react-markdown + remark-gfm + rehype-sanitize (see
 * src/components/MarkdownBody.tsx). No custom Markdown parser.
 *
 * Run: cd web/desktop && node --test tests/markdown.test.mjs
 */
...
test("headings, bold/italic, and lists render", () => {
  ...
  assert.ok(html.includes("<h1>Title</h1>"), `missing h1: ${html}`);
