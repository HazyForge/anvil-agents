...
// Local copy plugin (Node fs). Keep JS so tsconfig.node does not need @types/node.
// @ts-expect-error no bundled types for this helper
import { anvilAgentAvatars, repoRoot } from "./avatars-plugin.mjs";
...
export default defineConfig(({ mode }) => {
  ...
  return {
    plugins: [react(), anvilAgentAvatars()],
    base: "/",
    server: {
      port: 5173,
      fs: {
