...
export function validateSkillForm(form: SkillForm, isCreate: boolean): string | null {
  ...
  try { parseJSON(form.referencesJSON || "[]", "References"); } catch (error) { return String(error); }
  return null;
}
...
export function buildMCPSetSpec(form: MCPSetForm): Record<string, unknown> { const spec = { ...form.legacy }; delete spec.description; delete spec.serverRefs; if (form.description.trim()) spec.description = form.description.trim(); if (form.serverRefs.length) spec.serverRefs = form.serverRefs.map((name) => ({ name })); return spec; }
